...
contract UniswapIntegrationBase is Test {
  ...
  function setUp() public virtual {
    ...
    vault = new VaultManager(
      USDC, WETH, address(0), UNISWAP_V3_ROUTER_02, COMET_REWARDS_DUMMY, address(this)
    );
  ...
  function _executeSwap(ISwapRouter.ExactInputSingleParams memory params)
    external
...
contract Swap_USDC_AERO is UniswapIntegrationBase {
  function testFork_Swap_USDC_to_AERO() public {
    _swapAndAssert(USDC, AERO, 1000e6); // 1,000 USDC -> AERO
  }

  function testFork_Swap_AERO_to_USDC() public {
    _swapAndAssert(AERO, USDC, 1000e18); // 1,000 AERO -> USDC
  }
}
...
contract Swap_WETH_AERO is UniswapIntegrationBase {
  ...
  function testFork_Swap_AERO_to_WETH() public {
    _swapAndAssert(AERO, WETH, 1000e18); // 1,000 AERO -> WETH
  }
}
