...
/// @title VaultManager
/// @notice Manages portfolio funds with timelock, DeFi integrations, and redemption of WETH for MT
/// holders.
contract VaultManager is Ownable2Step, AccessControl, ReentrancyGuard, Pausable {
  ...
  constructor(
    ...
    if (
      _usdc == address(0) || _weth == address(0) || _uniswapV3Router == address(0)
        || _cometRewards == address(0)
    ) revert VaultManager__InvalidAddress();
    ...
    if (_agent != address(0)) _grantRole(AGENT_ROLE, _agent);
  }

  // --------------------
  // Uniswap v3 Integration
  // --------------------
  /// @notice Swap exact input amount via Uniswap v3 with the same signature as the router for tool
  /// compatibility.
  ...
  // --------------------
  /// @notice Supply an asset to its configured Comet market. Matches Comet's function
  /// name/signature.
  ...
  function _approveIfNeeded(address token, address spender, uint256 required) internal {
    ...
    if (current < required) IERC20(token).safeIncreaseAllowance(spender, required - current);
  }

  /// @notice Withdraw an asset from its configured Comet market. Matches Comet's function
  /// name/signature.
    ...
    returns (uint256)
  {
    ...
    uint256 claimed = ICometRewards(COMET_REWARDS).claim(comet, to, false);
  ...
  function getCurrentPhase() public view returns (Phase) {
    if (block.timestamp < UNLOCK_TIMESTAMP) return Phase.LOCKED;

    // Post-unlock: check if consolidated
    if (_isConsolidatedInternal()) return Phase.REDEMPTION;
  ...
  function _isConsolidatedInternal() internal view returns (bool) {
    ...
    for (uint256 j = 0; j < cometCount; j++) {
      ...
      if (allowedComets[comet] && IComet(comet).balanceOf(address(this)) > 0) return false;
    }
    return true;
  }
  ...
  function setAgent(address newAgent) external onlyOwner {
    ...
    if (agent != address(0)) _revokeRole(AGENT_ROLE, agent);
    agent = newAgent;
