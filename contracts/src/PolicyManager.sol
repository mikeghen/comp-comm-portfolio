...
contract PolicyManager is AccessControl, ReentrancyGuard {
    ...
    /// @dev Validates edit range, calculates cost, transfers USDC, mints MT tokens, and applies the edit.
    function editPrompt(uint256 start, uint256 end, string calldata replacement) external nonReentrant {
        // Validate edit range
        if (start > end || end > bytes(prompt).length) {
            revert PolicyManager__InvalidEditRange();
        }

        // Validate replacement length matches the range
        if (bytes(replacement).length != end - start) {
            revert PolicyManager__InvalidReplacementLength();
        }
